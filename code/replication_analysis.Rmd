...
Here we do some analysis on the effects that fall within the prediction interval.
```{r pred_interval_analysis,message=FALSE,warning=FALSE}
# Temporary data frame with the studies for which a 95% PI could be calculated
pi_dat_nona = pi_dat[rowSums(is.na(pi_dat))==0,]
# should be 92 x 11
dim(pi_dat_nona)
# Subset of effects that fall within 95\% PI
pi_in <- subset(pi_dat_nona, (cor_rep >= low & cor_rep <= high))
# should be 69 x 11
dim(pi_in)
# Of these, how many replication effect sizes were smaller than
# original?
sum(pi_in$cor_rep < pi_in$cor_orig)
# All original effects were positive...which replication effects within
# 95% PI were negative?
pi_in[which(pi_in$cor_rep <= 0),]
...
```{r p-vals}
# Assume cor_orig == rho
# Assume all statistics are F(1, df2)
...
get_ps <- function(cor_orig, cor_rep, n_orig, n_rep){
	...
	# Simulate 10,000 arcanh(r) assuming the original association is the true
	# one, but using the sample size of the replication study to compute the SD
	new_arc_rs <- rnorm(10000, mean_term, 1/sqrt(n_rep_tmp - 3))
...
df <- data.frame("cor_orig"=pi_dat_filt$cor_orig, "cor_rep"=pi_dat_filt$cor_rep, "psums"=p_sums)

colfunc <- colorRampPalette(c("darkorange1", "purple3"))
cols <- colfunc(10000)

layout(matrix(1:2,ncol=2), width = c(3,1))
...
legend("bottomright", legend=c("0%", "20%", "40%", "60%", "80%", "100%"), pch=19, col=colfunc(6), title = "% Rep. P<0.05")

#legend_image <- as.raster(matrix(rev(cols), ncol=1))
#plot(c(0,2),c(0,1),type = 'n', axes = F,xlab = '', ylab = '', main = '# P<0.05')
#text(x=1.5, y = seq(0,1,l=5), labels = seq(0,100,l=5))
#rasterImage(legend_image, 0, 0, 1,1)

## ggplot code for the same thing ("q" is the loess plot)
#p <- ggplot(df, aes(cor_orig, cor_rep))
#p <- p + scale_x_continuous(limits=c(0,1)) + scale_y_continuous(limits=c(-0.5, 1)) + geom_point(aes(color = psums), shape=19, size=4) + scale_color_gradient(low="darkorange1", high="purple3", name="# P < 0.05") + geom_abline(intercept=0, slope = 1, linetype=2) + geom_abline(intercept=0, slope = 0, linetype=2) + labs(title="Effect sizes colored by # P < 0.05 (out of 10,000 replicates)", x="", y = "Replication Effect Size") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(color = "black"))

#q <- qplot(data = subset(df,!is.na(df$cor_rep)) , cor_orig, p.vals, xlab = "Original Effect Size", ylab="# P < 0.05")
#q <- q + geom_smooth(method="loess", size=1.5, fill=NA) + scale_x_continuous(limits=c(0,1)) +scale_y_continuous(limits=c(0,100)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(color = "black"), plot.margin=unit(c(0,30,0,3), "mm"))

#pdf(file = "../results/pvals.pdf",family="Lato")
#grid.arrange(p, q, nrow=2, heights=c(3,1))
#dev.off()

# Can use this to make a violin plot of any iteration you want (out of 100)
#make_viol <- function(p.vals, orig_vals, num){
#	p <- p.vals[num,]
#	df2 <- data.frame("p"=c(orig_vals, p), "cat" = rep(c("Original Studies", "Replication"), c(length(orig_vals), length(p))))
#	t <- ggplot(df2, aes(cat, p))
#	t + geom_violin()
#}

#pdf(file = "../results/violin_plot.pdf",family="Lato")
#make_viol(p.vals, dat$T_pval..O, 12)
#dev.off()
```